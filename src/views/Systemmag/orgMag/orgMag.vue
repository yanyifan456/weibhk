...
const handleEdit = (record) => {
  console.log('编辑机构 - 当前记录:', record)
  currentRecord.value = record;
  editForm.id = record.id;
  editForm.name = record.name || "";
  editForm.deptCode = record.deptCode || "";
  editForm.userId = record.userId || "";
  editForm.showOrder = record.showOrder ?? 0;
  editForm.deptAddress = record.deptAddress || "";
  editForm.deptTel = record.deptTel || "";
  // 重置图片状态，保存原始图片URL
...
const handleEditCancel = () => {
  ...
  editForm.deptCode = "";
  editForm.userId = "";
  editForm.showOrder = 0;
  editForm.deptAddress = "";
  editForm.deptTel = "";
  editLogoFile.value = null;
  editLogoUrl.value = '';
...
const handleEditSubmit = async () => {
  ...
  if (!editForm.deptAddress || !editForm.deptAddress.trim()) {
    message.warning("機構地址不能為空");
    return;
  }
  if (!editForm.deptTel || !editForm.deptTel.trim()) {
    message.warning("機構地址不能為空");
    return;
  }
  ...
  try {
    const params = {
      ...
      showOrder: editForm.showOrder,
      deptAddress: editForm.deptAddress,
      deptTel: editForm.deptTel
    };
...
const handleAddSubmit = async () => {
  ...
  if (!addForm.deptAddress || !addForm.deptAddress.trim()) {
    message.warning({
      content: "機構地址不能為空",
      style: {
        zIndex: 3000
  ...
  if (!addForm.deptTel || !addForm.deptTel.trim()) {
    message.warning({
      content: "機構電話不能為空",
      style: {
        zIndex: 3000
  ...
  try {
    const params = {
      ...
      deptAddress: addForm.deptAddress,
      deptTel: addForm.deptTel
    };
