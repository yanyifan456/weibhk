<template>
    ...
    <div v-if="active" class="subtitle-panel">
        ...
        <div class="subtitle-toolbar">
            ...
            <div class="subtitle-status">
                <span class="status-dot" :class="{
                    'dot-connected': wsStatus === 'connected' && ws2Status === 'connected',
                    'dot-partial': (wsStatus === 'connected') !== (ws2Status === 'connected'),
                    'dot-connecting': wsStatus === 'connecting' || ws2Status === 'connecting',
                    'dot-disconnected': wsStatus === 'disconnected' && ws2Status === 'disconnected',
                }"></span>
        ...
        <div class="subtitle-content" ref="subtitleContentRef">
            <div v-for="(item, idx) in subtitleList" :key="idx" class="subtitle-item"
...
const props = defineProps({
    ...
    /** 当前用户 ID（医生端，如 doctor_16） */
    userId: {
        type: String,
        default: '',
    },
    /**
     * 另一方用户 ID（患者手机号），用于同时接收医生说的话
     * 后端按 targetUserId 路由消息，医生需同时订阅两个 userId 才能看到双方字幕
     */
    secondUserId: {
        type: String,
        default: '',
...
const ws2Status = ref('disconnected');
let subtitleWs = null;
let subtitleWs2 = null;

const statusText = computed(() => {
    const map = { connecting: '连接中...', connected: '已连接', disconnected: '未连接' };
    if (wsStatus.value === 'connected' && ws2Status.value === 'connected') return '双路已连接';
    if (wsStatus.value === 'connected') return '已连接(患者)';
    if (ws2Status.value === 'connected') return '已连接(医生)';
    if (wsStatus.value === 'connecting' || ws2Status.value === 'connecting') return '连接中...';
    return '未连接';
});
...
const createMessageHandler = () => (event) => {
    try {
        ...
        const subtitle = raw.subtitleData || raw;
...
const connectSubtitleWs = () => {
    ...
    // WS 2：接收医生自己说的话（targetUserId = 患者手机号）
    if (props.secondUserId) {
        const url2 = `${props.wsHost}/ws/subtitle/${props.roomId}/${props.secondUserId}`;
        ws2Status.value = 'connecting';
        subtitleWs2 = new WebSocket(url2);

        subtitleWs2.onopen = () => {
            ws2Status.value = 'connected';
        };
        subtitleWs2.onmessage = handler;
        subtitleWs2.onclose = () => {
            ws2Status.value = 'disconnected';
        };
        subtitleWs2.onerror = () => {
            ws2Status.value = 'disconnected';
        };
...
watch(
    () => [props.active, props.wsHost, props.roomId, props.userId, props.secondUserId],
